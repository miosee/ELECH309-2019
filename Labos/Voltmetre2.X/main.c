...
int main(void) {
    ...
    // Configuration de L'ADC pour declenchement par le TIMER3 sur AN0
    adcTimerInit();
    
    // Configuration du Timer3 pour deborder à 1kHz
	PR3 = 39999;                 // T3=1ms=(PR2+1)*25ns => PR2=39999
	T3CONbits.TON = 1;
    
	while(1) {
        // Plus besoin de polling sur le TIMER
        
        // Si l'ADC a fini une conversion, on lit le resultat et on l'affiche sur les LEDs
